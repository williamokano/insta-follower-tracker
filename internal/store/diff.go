...
func (s *Store) Diff(ctx context.Context, accountID int64, from, to Upload) (NetDiff, error) {
	diff := NetDiff{From: from, To: to}
	...
	lost, err := s.followers(ctx, "lost followers", followerSelect+`snapshot_members sm
		...
		WHERE sm.upload_id = ?
		  AND NOT EXISTS (SELECT 1 FROM snapshot_members o WHERE o.upload_id = ? AND o.user_id = sm.user_id)
		ORDER BY u.username`, from.ID, to.ID)
	if err != nil {
		return diff, err
	}
	diff.Lost = lost
	...
	gained, err := s.followers(ctx, "gained followers", followerSelect+`snapshot_members sm
		...
		WHERE sm.upload_id = ?
		  AND NOT EXISTS (SELECT 1 FROM snapshot_members o WHERE o.upload_id = ? AND o.user_id = sm.user_id)
		ORDER BY u.username`, to.ID, from.ID)
	if err != nil {
		return diff, err
	}
	diff.Gained = gained
	...
	returned, err := s.followers(ctx, "returned followers", followerSelect+`snapshot_members sm
		JOIN users u ON u.id = sm.user_id
		WHERE sm.upload_id = ?
		  AND EXISTS (SELECT 1 FROM snapshot_members o WHERE o.upload_id = ? AND o.user_id = sm.user_id)
		  AND EXISTS (
		...
		ORDER BY u.username`, from.ID, to.ID, accountID, fromSeq, toSeq)
	if err != nil {
		return diff, err
	}
	...
	transient, err := s.followers(ctx, "transient followers", `SELECT DISTINCT u.username, u.href
		...
		JOIN users u ON u.id = sm.user_id
		WHERE mu.account_id = ? AND mu.status = 'completed'
		  AND mu.sequence_no > ? AND mu.sequence_no < ?
		  AND NOT EXISTS (SELECT 1 FROM snapshot_members o WHERE o.upload_id = ? AND o.user_id = sm.user_id)
		  AND NOT EXISTS (SELECT 1 FROM snapshot_members o WHERE o.upload_id = ? AND o.user_id = sm.user_id)
		ORDER BY u.username`, accountID, fromSeq, toSeq, from.ID, to.ID)
	if err != nil {
		return diff, err
	}
...
func (s *Store) ChangesForUpload(ctx context.Context, uploadID int64, kind ChangeType) ([]Change, error) {
...
func (s *Store) AllUnfollowers(ctx context.Context, accountID int64) ([]Change, error) {
	return s.scanChanges(ctx, `SELECT u.username, u.href, c.change_type, c.upload_id, cu.sequence_no,
		       c.prev_upload_id, c.created_at
		FROM changes c
		JOIN users u ON u.id = c.user_id
		JOIN uploads cu ON cu.id = c.upload_id
		WHERE c.account_id = ? AND c.change_type = 'unfollowed'
		ORDER BY cu.sequence_no, u.username`, accountID)
}
